...
Решение
Реализуйте алгоритм бинарного поиска. С его помощью вы найдёте место в массиве, где слева от него будут элементы меньше или равны, а справа строго больше. Работает бинарный поиск за время O(log2n), дополнительную память O(1).
...
Напишите функцию, которая принимала бы массив, его размер, целое число (точку отсчёта) и возвращала бы количество бо́льших по размеру элементов по рассмотренному выше алгоритму. Внимание! Вызывать готовые реализации бинарного поиска запрещено.
Запросите у пользователя точку отсчёта.
Вызовите эту функцию, передав туда массив и точку отсчёта — 32. Выведите результат на экран. Убедитесь, что ответ верный — 3, так как только три элемента в массиве строго больше, чем 32.
Вызовите эту функцию, передав туда массив и точку отсчёта — 15. Выведите результат на экран. Убедитесь, что ответ верный — 8, так как только восемь элементов в массиве строго больше, чем 15).
...
int countGreaterElements(const int arr[], int size, int target) {
...
int main() {
    ...
    int count = countGreaterElements(arr, size, target);
