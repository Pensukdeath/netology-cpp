...
unsigned long long real_string_hash(const string& str, unsigned long long p, unsigned long long n) {
    unsigned long long hash_value = 0;
    unsigned long long p_pow = 1;
    
    for (size_t i = 0; i < str.length(); ++i) {
        hash_value = (hash_value + (static_cast<unsigned long long>(str[i]) * p_pow) % n) % n;
        p_pow = (p_pow * p) % n;
    }
